<?php
	...
	class CyberSource {
		...
		public function reverse_authorization ( $request_id, $amount = null, $reference  = null) {
			...
			$cc_auth_reversal_service->merchantReferenceCode = $reference;
			$request->ccAuthReversalService = $cc_auth_reversal_service;
			
			// merchant_ref_number

			$request->ccAuthReversalService = $cc_auth_reversal_service;
			$request->merchantReferenceCode = $reference;

			// if there was an amount specified, just use it - otherwise add the individual items
			// print_r($amount);die();
			if ( $amount !== null ) {
				$request->purchaseTotals->grandTotalAmount = $amount;
				// $request->purchaseTotals->currency = "DOP";
			}
			else {
				$this->create_items( $request );
			}
